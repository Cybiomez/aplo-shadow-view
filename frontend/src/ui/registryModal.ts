...
export async function initRegistryModal(h: Hooks): Promise<void> {
  ...
  overlay.innerHTML = `
    <div class="modal" role="dialog" aria-modal="true" aria-labelledby="regTitle">
      ...
      <div class="m-body">
        <div class="reg-note">Домен/логин/пароль для подключения к серверам. Пароли хранятся в Windows Credential Manager, в файле экспорта их нет. Домен — без слэша (напр. CORP); пусто = локальная учётка.</div>
...
async function addProfile(): Promise<void> {
  const cred = await credentialForm("Новая учётная запись");
  if (!cred || !cred.name) return;
  apply(await api.setProfile(cred.name, cred.domain, cred.username, cred.password, "domain"));
}

function credentialForm(title: string): Promise<{ name: string; domain: string; username: string; password: string } | null> {
    ...
    ov.innerHTML = `
      <div class="modal" role="dialog" aria-modal="true">
        ...
        <div class="m-body cred-form">
          <input class="txt" data-name placeholder="Название (напр. Домен-админ)" autocomplete="off" />
          <input class="txt" data-domain placeholder="Домен без слэша (пусто = локальная)" autocomplete="off" />
